...
## 2026-05-13 - Avoid Repeated JSONDecodeError in High-Frequency Streams
**Learning:** Calling `json.loads` on incomplete JSON strings inside a streaming loop causes significant overhead because Python exceptions (`JSONDecodeError`) are computationally expensive to raise and catch repeatedly.
**Action:** Use a fast string heuristic, such as `if not buffer.rstrip().endswith("}"): return None`, to bypass the parser attempt until the chunk appears structurally complete.
