...
## 2026-09-12 - Avoid JSONDecodeError overhead on incomplete chunks
**Learning:** When buffering streamed JSON chunks in a high-frequency loop, continually raising and catching JSONDecodeError exceptions introduces severe computational overhead.
**Action:** Use fast string heuristics, such as buffer.strip().endswith('}'), to bypass parsing attempts until the chunk appears structurally complete. Do not apply this heuristic to fully-formed JSON payloads.
